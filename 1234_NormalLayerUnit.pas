...
begin
    ...
    writeln('Input Length = '+itoa(layer.inputs));
    writeln('Mean = '+flots(layer.m));
    writeln('Std.dev = '+flots( layer.s));
    writeln('Coefficient = '+flots( layer.g));
    writeln('Constant = '+flots( layer.b));
    writeln('');
end;
...
begin
    for j := 0 to layer.inputs - 1 do
        layer.aout[j] := layer.g * ((xvec[j] - layer.m) / layer.s) + layer.b;
    result := layer.inputs;
end;
end.
