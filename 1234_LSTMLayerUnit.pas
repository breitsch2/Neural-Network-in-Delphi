...
type
    LSTMLayer = record
        ...
        aH: array of double;                                         { Hidden state cache matrix (h by cache) }
...
begin
    ...
    writeln('Input dimensionality d = '+itoa( layer.d));
    writeln('State dimensionality h = '+itoa( layer.h));
    writeln('State cache size       = '+itoa( layer.cache));

    writeln('Wi ('+flots( layer.h)+ ' x '+itoa( layer.d)+ ')');
    //Refactor------------------------>
    for i := 0 to layer.h - 1 do
 {   begin
        write('[');
...
begin
    ...
    begin                                                           //  Consider that we may have shifted states
        ...
        begin
            ht_1[n] := layer.aH[ n * layer.cache + t_1 ];
            ct_1[n] := layer.c[n];
        end;
    end;
    ...
    begin
        for n := 0 to layer.h - 1 do
        begin
     (*       i[n] := i[n]+ layer.Wi[n] * x[0];                             //  Add Wi dot x to i (i = W_i * x_t + U_i * h_{t-1} + b_i)
    ...
    begin
        ...
        begin
            for n := 0 to layer.h - 1 do
                layer.aH[n * layer.cache + m - 1] := layer.aH[n * layer.cache + m];
        end;
    end;
    ...
    begin
        ...
        layer.aH[ n * layer.cache + t ] := o[n] * ((2.0 / (1.0 + exp(-2.0 * layer.c[n]))) - 1.0);
    end;
    ...
    layer.t := layer.t+ 1;                                                   //  Increment time step
